...
public class SuppliersController : ControllerBase
{
    ...
    public SuppliersController(DataContext context, IFileStorage fileStorage,
        IConfiguration configuration, IUserHelper userHelper)
    {
        ...
        ImgRoute = "wwwroot\\Images\\ImgSuppliers";
    }

    [HttpGet("loadCombo")]
    public async Task<ActionResult<IEnumerable<Supplier>>> GetCorporations()
    {
        string email = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Name)!.Value;
        User user = await _userHelper.GetUserAsync(email);

        var listResult = await _context.Suppliers.Where(x => x.Active && x.CorporationId == user.CorporationId).ToListAsync();
        return listResult;
    }

    // GET: api/Corporations
    [HttpGet]
    public async Task<ActionResult<IEnumerable<Supplier>>> GetSuppliers([FromQuery] PaginationDTO pagination)
    {
        string email = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Name)!.Value;
        User user = await _userHelper.GetUserAsync(email);

        var queryable = _context.Suppliers.Where(x => x.CorporationId == user.CorporationId)
    ...
    [HttpPut]
    public async Task<IActionResult> PutSuppliers(Supplier modelo)
    {
        try
        {
            ...
            _context.Suppliers.Update(modelo);
    ...
    // To protect from overposting attacks, see https://go.microsoft.com/fwlink/?linkid=2123754
    [HttpPost]
    public async Task<ActionResult<Supplier>> PostSuppliers(Supplier modelo)
    {
        try
        {
            string email = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Name)!.Value;
            ...
            await transaction.CommitAsync();
            return CreatedAtAction("GetSuppliers", new { id = modelo.SupplierId }, modelo);
        }
        catch (DbUpdateException dbUpdateException)
        {
            if (dbUpdateException.InnerException!.Message.Contains("duplicate"))
