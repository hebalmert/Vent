...
public partial class IndexProductStorage
{
    ...
    private const string baseUrl = "api/productStorages";
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = await _dialogService.ShowAsync<EditProductStorage>($"Editar Bodega", parameters, options);
        }
        else
        {
            dialog = await _dialogService.ShowAsync<CreateProductStorage>($"Nueva Bodega", options);
        }
