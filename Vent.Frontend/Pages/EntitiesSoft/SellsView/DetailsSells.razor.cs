...
public partial class DetailsSells
{
    ...
    private const string baseUrl = "/api/sellsDetails";

    public Sell? Sell { get; set; }
    public List<SellDetails>? SellDetails { get; set; }

    [Parameter] public int Id { get; set; }  //Codigo del SellId
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttpCountry = await _repository.GetAsync<Sell>($"/api/sells/{Id}");
        ...
        var responseHttp = await _repository.GetAsync<List<SellDetails>>(url);
        // Centralizamos el manejo de errores
        ...
        Sell = responseHttpCountry.Response;
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        else
        {
            ...
            dialog = await _dialogService.ShowAsync<CreateSellDetails>($"Nuevo Item", parameters, options);
        }
    ...
    private async Task ClosePurchaseAsync(int id)
    {
        ...
        {
            Title = "Desea Cerrar Venta",
            Text = "¿Al Cerrar la Venta, no podra volver editar y los Inventarios se actualizaran, Continuar?",
        ...
        var responseHttp = await _repository.PostAsync($"{baseUrl}/CerrarSells", Sell);
